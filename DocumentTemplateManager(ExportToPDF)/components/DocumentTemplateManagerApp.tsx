...
export const DocumentTemplateManagerApp: React.FunctionComponent<IDocumentTemplateManagerProps> = (props: IDocumentTemplateManagerProps) => {
    ...
    const FetchTemplates = async () => {
        ...
        const currentUsername = props.pcfContext.userSettings.userName;
        const recordId = (props.pcfContext.mode as any).contextInfo.entityId;
        const tableLogicalName = props.primaryEntityName;
        console.log(`Logical Name of the Table: ${tableLogicalName}`);
        console.log(`DownloadBy: ${props.pcfContext.parameters.DownloadBy.raw}`);
        console.log(`DownloadDate: ${props.pcfContext.parameters.DownloadDate.raw}`);
        if (!recordId) {
            console.error("RecordId not available.");
            return;
        }
        const dataForUpdate = {
            [String(props.pcfContext.parameters.DownloadDate.raw)]: currentDate,
            [String(props.pcfContext.parameters.DownloadBy.raw)]: currentUsername,
        };
        try {
            // Perform the update using the Web API
            await props.pcfContext.webAPI.updateRecord(tableLogicalName, recordId, dataForUpdate);
    ...
    const CreateEmail = async () => {
        ...
        email["subject"] = (props.pcfContext.mode as any).contextInfo.entityRecordName;
        email["email_activity_parties"] = [{
